...
export function buildServer(): McpServer {
  ...
  server.prompt(
    ...
    { handle: z.string().describe("Codeforces handle (optional; defaults to CP_MCP_CF_HANDLE)").optional() },
    ({ handle }) => {
      const resolvedHandle = resolveHandle("codeforces", handle);
      return {
        messages: [
          {
            role: "user",
            content: {
              type: "text",
              text: `Generate a daily Codeforces practice ladder for the user '${resolvedHandle}'.
Please do the following:
1. Run cp_analyze_weaknesses_codeforces for '${resolvedHandle}' to identify their weakest tags.
...
3. Present the 3-5 selected problems as today's ladder.`
  ...
  server.prompt(
    ...
    { handle: z.string().describe("AtCoder handle (optional; defaults to CP_MCP_AC_HANDLE)").optional() },
    ({ handle }) => {
      const resolvedHandle = resolveHandle("atcoder", handle);
      return {
        messages: [
          {
            role: "user",
            content: {
              type: "text",
              text: `Generate a daily AtCoder practice ladder for the user '${resolvedHandle}'.
Please do the following:
1. Run cp_analyze_weaknesses_atcoder for '${resolvedHandle}' to identify their weakest difficulty bands.
...
3. Present the 3-5 selected problems as today's ladder.`
  ...
  server.prompt(
    ...
    {
      handle: z.string().describe("Codeforces handle (optional; defaults to CP_MCP_CF_HANDLE)").optional(),
      problems: z.string().describe("Comma-separated list of problem IDs")
    },
    ({ handle, problems }) => {
      const resolvedHandle = resolveHandle("codeforces", handle);
      return {
        messages: [
          {
            ...
            content: {
              type: "text",
              text: `Audit yesterday's Codeforces ladder for '${resolvedHandle}'.
        ...
        messages: [
          {
            ...
            content: {
              type: "text",
              text: `Audit yesterday's AtCoder ladder for '${resolvedHandle}'.
The assigned problems were: ${problems}.
